...
def benchmark_algorithm(alg: str, bits: List[int], repeats: int = 5) -> pd.DataFrame:
    ...
    for bit_size in bits:
        ...
        for i in range(repeats):
            print(f"  第 {i + 1} 次重复...")
            if alg == 'rsa':
                keygen_time, enc_time, dec_time = _rsa_ops(bit_size)
            elif alg == 'elgamal':
                keygen_time, enc_time, dec_time = _elgamal_ops(bit_size)
            else:
...
def _rsa_ops(size: int) -> tuple[float, float, float]:
    ...
    # 准备测试消息
    msg = b"Test message for benchmarking RSA."
...
def _elgamal_ops(size: int) -> tuple[float, float, float]:
    ...
    # 准备测试消息
    msg = b"Test message for benchmarking ElGamal."
